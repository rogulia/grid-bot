...
class BybitClient:
    ...
    def get_public_trade_history(
        ...
        """
        Get public trade history (recent trades)

        This returns actual executed trades on the exchange, useful for backtesting
        with tick-by-tick precision.
        ...
        Returns:
            List of trade records, each containing:
                ...
                - isBlockTrade: Whether this is a block trade

        Example:
            >>> # Get last 1000 trades
            >>> trades = client.get_public_trade_history('SOLUSDT')
        ...
        try:
            ...
            response = self.session.get_public_trade_history(**params)

            if response.get('retCode') == 0:
                trades = response.get('result', {}).get('list', [])
                self.logger.debug(
                    f"Retrieved {len(trades)} public trades for {symbol}"
                )
                return trades
            else:
                self.logger.error(f"Failed to get public trade history: {response}")
                return []

        except Exception as e:
            self.logger.error(f"Error getting public trade history: {e}")
            return []
    ...
    def get_kline(
        ...
        """
        Get historical klines/candlesticks (OHLCV data)

        This provides historical price data over extended periods, useful for backtesting.
        Bybit stores klines for up to 200 days (depending on interval).

        Args:
            symbol: Trading symbol (e.g., 'SOLUSDT')
            interval: Kline interval - 1,3,5,15,30,60,120,240,360,720,D,M,W
                     (1 = 1 minute, 60 = 1 hour, D = 1 day, etc.)
            category: Market category (default: 'linear')
        ...
        Returns:
            List of kline records, each containing:
                - startTime: Kline start time (timestamp in ms)
        ...
        Example:
            >>> # Get 1-minute klines for last 7 days
            >>> klines = client.get_kline('SOLUSDT', interval='1', limit=1000)
            >>> print(f"Retrieved {len(klines)} candles")
            >>> for kline in klines[:3]:
            ...     print(f"Time: {kline['startTime']}, O: {kline['openPrice']}, C: {kline['closePrice']}")
        """
        try:
            params = {
                "category": category,
            ...
            if response.get('retCode') == 0:
                ...
                self.logger.debug(
                    f"Retrieved {len(klines)} klines for {symbol} (interval: {interval})"
                )
                return klines
            else:
