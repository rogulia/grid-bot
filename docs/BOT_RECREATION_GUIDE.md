# Руководство по воссоздаю бота GRID-Trader с нуля
...
При запуске скрипта бот должен:
 - удалить все невыполненные ордера с биржи (тейки и лимитки)
 - очистить файл стейт бота
 - восстановить активные ордера с биржи по REST API в стейт бота
 - поставить тейки по описанным ниже правилам и записать их ID в стейт бота
 - поставить недостающие лимитки чтобы сбалансировать qty обоих сторон и записать их в стейт бота

Как восстанавливать ордера? Вот нинструкция для лонгов, для шоротв аналогичная:
...
- Нужно проверить достаточность баланса:
 ...
 - важно, что при наличии денег у позиций должен быть открыт лимитны ордер на усреднение с двойным объемом относительно предыдущего ордера и на расстоянии 1% от предыдущего ордера как минимум на 1 уровне от последнего ордера
...
- лимитные ордера должны открываться симмитрично для лонг и шорт, либо не открываться вообще
...
#### Take Profit (Фиксация прибыли)
 ...
 - после исполнения тейк профита необходимо:
   ...
   - тут же открыть новую позицию по лимитке на уровне противоположной позиции минус два шага (количество исполненных ордеров по противоположней позиции отражено в стейте бота). Если нужно открыться сразу на несколько уровней, то каждый уровень должен открываться отдельным ордером чтобы сохранить последовательность ордеров (например, если противоположная позиция находится на 8 уровне, то текущую надо открыть на 6 уровне, каждый уровень открывая отдельно: 1 + 2 + 4 + 8 + 16 + 32 + 64)
   - не недостающие 2 уровня надо открыть лимитки с удвоением объема на каждом шагу на расстоянии 1% сохраняя сетку
...
5. **Восстановление состояния (критически важно!)**
   ...
   - Если во время выполнения синхронизации что-то пришло на вебсокет о том, что какая-то позиция изменилась или отменилась или ещё что-то случилось, то снова очистить стейт бота и выполнить синхронизацию ещё раз чтобы на момент чтения вебсокета синхронизация уже была бы выполнена и все ордера были бы отражены в стейте бота
