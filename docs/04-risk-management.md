...
**Версия:** 3.2 (Логика балансировки позиций)
**Статус:** Исправлена фундаментальная ошибка в понимании panic mode

**🔥 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ V3.2 (13 октября 2025):**
- ✅ **ЕДИНСТВЕННОЕ ПРАВИЛО**: Денег ВСЕГДА должно хватать для балансировки позиций
- ✅ **Panic Mode**: Проверяет `available >= imbalance × 1.15` (реальная стоимость балансировки)
- ✅ **Early Freeze**: Использует ту же логику (упрощена)
- ✅ **Перед усреднением**: СИМУЛЯЦИЯ - проверяет что ПОСЛЕ него будет хватать на балансировку
- ✅ Удалены все процентные/абсолютные/множительные проверки - остался только реальный расчет
- ✅ Работает корректно на любом размере баланса и уровне дисбаланса
...
def check_early_freeze_trigger() -> bool:
    ...
    ЕДИНСТВЕННОЕ ПРАВИЛО: Денег должно хватать для балансировки позиций
    
    Early Freeze = та же проверка что и Panic Mode.
    Разница только в действиях (freeze vs panic), но не в триггере.

    Returns:
        True если нужно freeze, False если все ОК
    """
    # Early Freeze использует ТУ ЖЕ логику что и Panic Mode
    return check_panic_trigger_low_im()


def check_panic_trigger_low_im() -> bool:
    """
    Проверка: хватает ли денег для балансировки позиций?
    
    CORE LOGIC: В хедж-стратегии сбалансированные позиции = защита от ликвидации.
    Должны ВСЕГДА иметь возможность выровнять long_margin = short_margin.
    """
    # 1. Получить баланс
    total_balance = balance_manager.get_available_balance()
    
    # 2. Рассчитать маржу в каждом направлении (ВСЕ символы)
    total_long_margin = 0
    ...
    for strategy in strategies:
        for pos in strategy.pm.long_positions:
            ...
            position_margin = position_value / strategy.leverage
            total_long_margin += position_margin
        
        for pos in strategy.pm.short_positions:
            position_value = pos.quantity * strategy.current_price
            position_margin = position_value / strategy.leverage
            total_short_margin += position_margin
    
    # 3. Дисбаланс позиций
    imbalance = abs(total_long_margin - total_short_margin)
    
    # 4. Доступные средства
    available = total_balance - total_long_margin - total_short_margin
    
    # 5. Нужно для балансировки (+ 15% на комиссии/фандинг/слиппедж)
    required_to_balance = imbalance * 1.15
    
    # 6. ПРОВЕРКА
    if available < required_to_balance:
        return True  # FREEZE/PANIC - не можем выровнять позиции!
    
    return False  # Всё ОК


# Пример 1: Стабильная ситуация
# Balance: $1000
# Long margin: $400, Short margin: $450
# Imbalance: $50
# Available: $1000 - $400 - $450 = $150
# Required: $50 × 1.15 = $57.50
# $150 > $57.50 ✅ → OK, не freeze

# Пример 2: Перезапуск с позициями
# Balance: $787
# Long margin: $144, Short margin: $143
# Imbalance: $1
# Available: $787 - $144 - $143 = $500
# Required: $1 × 1.15 = $1.15
# $500 > $1.15 ✅ → OK, не паникуем!
```
...
|----------|-------------|------------|
| **Триггер** | `available < imbalance × 1.15` | `available < imbalance × 1.15` |
| **Логика** | ✅ ОДИНАКОВАЯ (денег хватит на балансировку?) | ✅ ОДИНАКОВАЯ (денег хватит на балансировку?) |
...
| **Балансировка?** | ❌ НЕТ (ждем TP) | ✅ ДА (выравниваем немедленно) |
| **Серьезность** | 🟡 Предупреждение | 🔴 Критическая ситуация |

**Почему одинаковый триггер:**
- В обоих случаях проблема одна: **не можем выровнять позиции**
- Разница только в реакции: Early Freeze ждет TP, Panic балансирует принудительно
- Это упрощает логику и делает систему предсказуемой

**Преимущества логики балансировки:**
- ✅ Работает корректно при любом размере баланса и дисбалансе
- ✅ Учитывает **реальную стоимость** выравнивания позиций
- ✅ Не паникует при сбалансированных позициях (даже на малом балансе)
- ✅ Сохраняет TP ордера при стабильных позициях

**Пример работы:**

```python
# Сценарий 1: Стабильные позиции
Balance: $1000
Long margin: $400
Short margin: $450
Imbalance: $50
...
$150 > $57.50 ✅ → Всё OK, не freeze, не паника

# Сценарий 2: После усреднений, но позиции близки
Balance: $1000
Long margin: $480
Short margin: $490
Imbalance: $10
Available: $30
Required: $10 × 1.15 = $11.50

$30 > $11.50 ✅ → Всё OK! Баланс маленький, но позиции почти равны

# Сценарий 3: Большой дисбаланс
Balance: $1000
Long margin: $200
Short margin: $750
Imbalance: $550
Available: $50
Required: $550 × 1.15 = $632.50

$50 < $632.50 ❌ → FREEZE/PANIC! Не можем выровнять позиции!

# Сценарий 4: Перезапуск с открытыми позициями (реальный случай)
Balance: $787
Long margin: $144
Short margin: $143
Imbalance: $1 (почти идеальный баланс!)
Available: $500
Required: $1 × 1.15 = $1.15

$500 > $1.15 ✅ → Не паникуем!
TP ордера создаются и сохраняются! ✅
```
...
def check_panic_trigger_low_im() -> (bool, str):
    """
    Проверка триггера: не можем выровнять позиции

    ЕДИНСТВЕННАЯ ПРОВЕРКА: Хватит ли денег для балансировки позиций?
    
    В хедж-стратегии сбалансированные позиции = защита от ликвидации.
    Должны ВСЕГДА иметь возможность выровнять long_margin = short_margin.
    ...
    if available < required_to_balance:
        reason = (
            f"LOW_IM: available=${available:.2f} < required_to_balance=${required_to_balance:.2f} "
            f"(imbalance=${imbalance:.2f}, long=${total_long_margin:.2f}, short=${total_short_margin:.2f})"
        )
        return (True, reason)
...
# Пример 1: Сбалансированные позиции (даже на малом балансе)
# Balance: $100
# Long margin: $45, Short margin: $45
# Imbalance: $0
# Available: $10
# Required: $0 × 1.15 = $0
# $10 > $0 ✅ → НЕ паника (позиции идеально сбалансированы!)

# Пример 2: Перезапуск с позициями (реальный случай)
# Balance: $787
# Long margin: $144, Short margin: $143
# Imbalance: $1
# Available: $500
# Required: $1 × 1.15 = $1.15
# $500 > $1.15 ✅ → НЕ паника, TP ордера сохраняются!

# Пример 3: Большой дисбаланс - нужна паника
# Balance: $1000
# Long margin: $200, Short margin: $750
# Imbalance: $550
# Available: $50
# Required: $550 × 1.15 = $632.50
# $50 < $632.50 ❌ → ПАНИКА! Не можем выровнять!
```
